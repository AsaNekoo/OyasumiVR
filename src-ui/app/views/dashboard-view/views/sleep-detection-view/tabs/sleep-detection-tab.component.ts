...
export function getStringForDurationAwake(translate: TranslateService, duration: string): string {
  const [hours, minutes] = duration.split(':').map((v) => parseInt(v));
  if (hours && minutes) {
    return translate.instant(
      'sleep-detection.disableAutomations.awakeFor.description.hoursAndMinutes',
      {
        hours,
        minutes,
      }
    ...
    return translate.instant('sleep-detection.disableAutomations.awakeFor.description.hours', {
      hours,
    });
  } else if (minutes) {
