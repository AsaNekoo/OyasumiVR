...
use openvr::input::VRInputValueHandle as InputValueHandle;
use openvr::system::DeviceActivityLevel;
...
pub async fn on_ovr_event(event: openvr::system::EventInfo) {
    match event.event {
        ...
        openvr::system::Event::PropertyChanged(tracked_device_property) => {
            ...
            if matching_properties.contains(&tracked_device_property.property) {
                update_device(event.tracked_device_index, true).await;
            }
        }
        _ => {}
...
async fn update_all_devices(emit: bool) {
    for n in 0..(openvr_sys::k_unMaxTrackedDeviceCount as usize) {
        update_device(n.try_into().unwrap(), emit).await;
    }
}

async fn update_device(device_index: TrackedDeviceIndex, emit: bool) {
    let context = OVR_CONTEXT.lock().await;
    ...
    if class == TrackedDeviceClass::Invalid && !device_class_cache.contains_key(&device_index) {
        return;
    }
    // Update class cache
    if class == TrackedDeviceClass::Invalid {
        device_class_cache.remove(&device_index);
    } else {
        device_class_cache.insert(device_index, class.clone());
    }
    drop(device_class_cache);

    let handle_type: Option<OVRHandleType> = device_handle_cache.get(&device_index).cloned();
    ...
    let can_power_off: Option<bool> = system
        .get_tracked_device_property_bool(device_index, openvr::property::DeviceCanPowerOff_Bool)
        .ok();
    let is_charging: Option<bool> = system
        .get_tracked_device_property_bool(device_index, openvr::property::DeviceIsCharging_Bool)
        .ok();
    ...
    let serial_number: Option<String> = system
        .get_tracked_device_property_string(device_index, openvr::property::SerialNumber_String)
        .ok();
    let hardware_revision: Option<String> = system
        .get_tracked_device_property_string(device_index, openvr::property::HardwareRevision_String)
        .ok();
    let manufacturer_name: Option<String> = system
        .get_tracked_device_property_string(device_index, openvr::property::ManufacturerName_String)
        .ok();
    let model_number: Option<String> = system
        .get_tracked_device_property_string(device_index, openvr::property::ModelNumber_String)
        .ok();
    ...
    if class == TrackedDeviceClass::HMD {
        ...
        display_frequency = system
            .get_tracked_device_property_f32(device_index, openvr::property::DisplayFrequency_Float)
            .ok();
    }
...
async fn detect_inputs() {
    ...
    for action in input_ctx.actions.iter() {
        match input.get_digital_action_data(
            action.handle,
            InputValueHandle(openvr_sys::k_ulInvalidInputValueHandle),
        ) {
            Ok(data) => {
                if data.0.bChanged {
